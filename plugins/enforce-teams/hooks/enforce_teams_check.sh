#!/bin/bash
# Enforce Teams UserPromptSubmit Hook
# Deterministic enforcement — runs on every user prompt, unlike CLAUDE.md which is advisory.
# Reads the current autonomy level from ~/.claude/enforce-teams-level

LEVEL_FILE="$HOME/.claude/enforce-teams-level"
